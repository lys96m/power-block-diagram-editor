...
- テキスト入力（ラベルや数値）はデバウンス付きで反映する（現状 300ms）。入力中の検証負荷を抑えつつ、blur 時には即時コミットする。
- セレクトボックスやトグルなどの選択系は即時反映でよい（デバウンス不要）。
- 長文/JSON などの入力は明示的なボタン操作で反映する（例: ProjectDialog の Load/Copy）。自動保存は行わない。
